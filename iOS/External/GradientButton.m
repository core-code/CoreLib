...
- (CGGradientRef)disabledGradient
{
    if (disabledGradient == NULL)
    {
        ...
        CGColorSpaceRef space = CGColorSpaceCreateDeviceRGB();
            ...
            NSMutableArray *disabled = [NSMutableArray arrayWithCapacity:3];
...
- (void)awakeFromNib
{
    if ([self tag] == 1)
        ...
        [self useWhiteActionSheetStyle];
    else if ([self tag] == 6)
        [self useBlackActionSheetStyle];
    else if ([self tag] == 7)
...
- (void)layoutSubviews
{
    ...
    imageFrame.origin.x = self.frame.size.width - imageView.frame.size.width - 10;
...
- (void)useWhiteActionSheetStyle
{
    ...
    self.strokeColor = [UIColor colorWithRed:0.67 green:0.67 blue:0.67 alpha:1.0];
...
- (void)drawRect:(CGRect)rect 
{
    ...
    if (self.enabled)
        ...
        [[UIColor colorWithRed:0.67 green:0.67 blue:0.67 alpha:1.0] setStroke];
